...
access = driveService.permissions().create(
    ...
    body = {'type': 'user', 'role': 'writer', 'emailAddress': '174fcsm@gmail.com'},  # Открываем доступ на редактирование
    fields = 'id'
).execute()
...
results = service.spreadsheets().values().batchUpdate(spreadsheetId = "1DFccIzNTm-hZ058HI49Fjh67_nXU-lpcEOaHPzE4uHo", body = {
    ...
    "data": [
        {"range": "Лист1!A4:I4",
         ...
         "values": [
                    [clinic_id, "test", str(len(test_old_names)), str(len(test_second_names)), str(len(prejnih_dict)), str(len(udaleno)), str(len(dobavleno)), str(len(izmeneno_dict)), clinic_name], # Заполняем первую строку
                   ]}
    ]
}).execute()
'''
for i in range(2, 100):
  ...
  try:
    ...
    time.sleep(1)
    ranges = ["Лист1!A" + str(value_were_stop) + ":I" + str(value_were_stop)] # 
    results = service.spreadsheets().values().batchGet(spreadsheetId = "1DFccIzNTm-hZ058HI49Fjh67_nXU-lpcEOaHPzE4uHo", 
                                         ranges = ranges, 
                                         valueRenderOption = 'FORMATTED_VALUE',  
                                         dateTimeRenderOption = 'FORMATTED_STRING').execute()
